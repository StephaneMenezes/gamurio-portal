...
function handleEscalaChange(radio) {
  const wrap     = document.getElementById('outraEscalaWrap');
  const textarea = document.getElementById('outra_escala');
...
document.getElementById('escalaForm').addEventListener('submit', async function (e) {
  ...
  const payload = {
    ...
    tipo_escala:   form.querySelector('input[name="tipo_escala"]:checked')?.value || '',
